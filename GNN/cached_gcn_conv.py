...
class CachedGCNConv(MessagePassing):
    ...
    @staticmethod
    def norm2(edge_index, num_nodes, edge_weight=None, improved=False,
             dtype=None):
        if edge_weight is None:
            edge_weight = torch.ones((edge_index.size(1), ), dtype=dtype,
                                     device=edge_index.device)
    ...
    def forward(self, x, edge_index, use_ppmi=True, cache_name="default_cache",edge_weight=None):
        """"""
        ...
        if use_ppmi:
            ...
            edge_index, norm = self.norm2(edge_index, x.size(0), edge_weight,
                                         self.improved, x.dtype)
            edge_index=edge_index.to('cuda')
            norm = norm.to('cuda')
            return self.propagate(edge_index, x=x, norm=norm)

        # print("cached_gcn/edge_index_device:",edge_index.device)
        # print("cached_gcn/x_device:", x.device)
        # print("cached_gcn/norm_device:", norm.device)
